<script lang="ts">
	...
	import { signIn, TypeValidate, validateForm } from '$lib/auth';
	...
	onMount(() => {
		...
		if (elementFormButton instanceof HTMLButtonElement) {
			elementFormButton.addEventListener('click', async (ev) => {
				...
				const { isValid, errors } = validateForm(TypeValidate.SIGNIN, user);
...
<form
	class="mx-[calc(calc(100vw-400px)/2)] my-[calc(calc(100vh-325px)/2)] h-81.25 w-100 gap-2 rounded-2xl bg-ayu-panel p-8 [&>div]:flex [&>div]:flex-col [&>div]:justify-center"
>
	<div>
		<h2 class="text-3xl font-bold text-ayu-fg">Sign In</h2>
	...
	<div class="row-span-2 grid grid-rows-2">
		...
		<section class="flex h-full w-full items-center justify-center text-ayu-fg">
			New here? &nbsp;
			<a href="/signup" class="text-ayu-orange transition-colors duration-400 hover:text-ayu-accent"
...
<style lang="postcss">
	...
	@layer components {
		form {
			display: grid;
			grid-template-rows: 40px repeat(2, 1fr) repeat(2, 40px);
